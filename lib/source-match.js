...
function loadMatch() {
	...
	try {
		...
		// 跨源只在官方地址不可用时触发；同时比较可用候选并选择码率最高者，
		// 避免“第一个能播”把低质量个人上传抢在正规音源前面。
		process.env.SELECT_MAX_BR = '1';
...
/**
 * 用实际音频大小与码率反算时长，只接受质量和版本时长都可验证的候选。
 * 这样即使上游搜索退回第一条，也不会把明显不同版本交给播放器。
 */
export function qualifyMatchedAudio(result, song) {
	if (!result || typeof result.url !== 'string' || !/^https?:\/\//i.test(result.url)) return null;
	const bitrate = Number(result.br) || 0;
	const size = Number(result.size) || 0;
	const expectedDuration = Number(song?.dt ?? song?.durationMs ?? song?.duration) || 0;
	if (bitrate < 128000 || size <= 0 || expectedDuration <= 0) return null;
	const calculatedDuration = size * 8 * 1000 / bitrate;
	const tolerance = Math.max(5000, expectedDuration * 0.02);
	if (!Number.isFinite(calculatedDuration) || Math.abs(calculatedDuration - expectedDuration) > tolerance) return null;
	return {
		url: result.url,
		source: String(result.source || ''),
		bitrate,
		durationMs: Math.round(calculatedDuration)
	};
}
...
export async function matchSourceUrl(song, platforms) {
	...
	try {
		...
		return qualifyMatchedAudio(result, song)?.url || null;
	} catch {
		return null;
	}
}
