 ...
 *
 * 记录入口复用客户端的播放进度上报（/dsh-alger/playback，每 2s 一次）：
...
export function createHabits(options = {}) {
	...
	let session = { songId: null, lastPos: 0, duration: 0 };
	...
	async function recordPlayback(info) {
		...
		if (!song || !song.id) { session.songId = null; return false; }
		const sid = String(song.id);
		...
		if (session.songId !== sid) {
			if (session.songId && session.duration > 0 && session.lastPos >= session.duration * 0.9) {
				...
				if (prev) prev.completed = (prev.completed || 0) + 1;
			}
	...
	async function clear() {
		if (!loaded) await load();
		data = emptyData();
		session = { songId: null, lastPos: 0, duration: 0 };
		await flush();
		return true;
	}
