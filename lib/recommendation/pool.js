...
import { isPlaceholderArtist } from './identity.js';
...
function validTrack(track) {
	return track && typeof track.trackKey === 'string' && track.trackKey.trim() &&
		typeof track.title === 'string' && track.title.trim() &&
		Array.isArray(track.artists) && track.artists.length > 0 &&
		track.artists.every((artist) => typeof artist === 'string' && artist.trim() && !isPlaceholderArtist(artist));
}
...
function sanitizeItems(items) {
	...
	for (const track of Array.isArray(items) ? items : []) {
		if (!validTrack(track) || keys.has(track.trackKey)) continue;
		keys.add(track.trackKey);
		result.push(cleanTrack(track));
	}
	return result;
}

function sanitizeLoadedState(value, historySize) {
	if (!value || value.version !== VERSION || !Array.isArray(value.items) || !Array.isArray(value.recentRecommendedTrackKeys)) return null;
	const items = sanitizeItems(value.items);
	const recentRecommendedTrackKeys = [...new Set(value.recentRecommendedTrackKeys
		.filter((key) => typeof key === 'string' && key.trim())
		.map(String))].slice(-historySize);
	const pendingTracks = sanitizeItems(value.pending?.tracks);
	const pending = value.pending && typeof value.pending.token === 'string' && pendingTracks.length > 0
		? { token: value.pending.token, tracks: pendingTracks }
		: null;
	const next = {
		...emptyState(),
		...
		items,
		recentRecommendedTrackKeys,
		lastGenerationStatus: value.lastGenerationStatus ?? null,
		pending
	};
	return {
		state: next,
		changed: JSON.stringify(next) !== JSON.stringify({ ...emptyState(), ...value })
	};
}
...
export function createRecommendationPool(options = {}) {
	...
	async function ensureLoaded() {
		...
		try {
			...
			const sanitized = sanitizeLoadedState(parsed, historySize);
			if (sanitized) {
				state = sanitized.state;
				migrated = sanitized.changed;
			}
		} catch {
			state = emptyState();
		}
	...
	async function consume(count = batchSize, options = {}) {
		return exclusive(async () => {
			...
			const excluded = new Set((options.excludeTrackKeys ?? []).filter(Boolean).map(String));
			const tracks = state.items.filter((track) => !excluded.has(track.trackKey)).slice(0, requested);
			...
			const consumedKeys = new Set(tracks.map((track) => track.trackKey));
			state.items = state.items.filter((track) => !consumedKeys.has(track.trackKey));
