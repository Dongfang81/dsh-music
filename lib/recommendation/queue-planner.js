...
function isPureExploration(entry) {
	const origins = entry?.track?.origins ?? [];
	return origins.length === 1 && origins[0] === 'exploration';
}

export function planQueue({ ranked = [], targetSize = 15, rng = Math.random, currentTrack = null, existingQueue = [], maxExplorationRatio = 1 } = {}) {
	const target = Math.max(0, Math.floor(Number(targetSize) || 0));
	const explorationRatio = Math.max(0, Math.min(1, Number(maxExplorationRatio) || 0));
	const explorationLimit = Math.floor(target * explorationRatio);
	...
	function insertionIndex(entry, artistCap = 2) {
		if (isPureExploration(entry) && explorationCount >= explorationLimit) return -1;
	...
	function take(entry, index = selected.length) {
		...
		if (isPureExploration(entry)) explorationCount += 1;
