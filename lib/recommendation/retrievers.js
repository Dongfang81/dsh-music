import { dedupeTracks, isPlaceholderArtist, normalizeTrack } from './identity.js';
...
export async function retrieveCurrentSimilar({ context, client, signal }) {
	...
	const artist = String(current.artists?.[0] ?? '').trim();
	if (!title || !artist || isPlaceholderArtist(artist)) return [];
	...
	return search(client, `${artist} ${title}`, 20, signal);
}
