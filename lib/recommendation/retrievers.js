...
async function search(client, query, limit = 20) {
	...
	return songsFrom(await client.search(String(query).trim(), 1, limit));
}

async function playlistCandidates(client, keyword, playlistLimit = 3, trackLimit = 25) {
	if (!client || typeof client.search !== 'function' || typeof client.playlist !== 'function') return [];
	const result = await client.search(keyword, 1000, playlistLimit);
	const playlists = asArray(result?.playlists).slice(0, playlistLimit);
	const settled = await Promise.allSettled(playlists.map((item) => client.playlist(item.id, trackLimit)));
	return settled.flatMap((item) => item.status === 'fulfilled' ? asArray(item.value?.tracks).slice(0, trackLimit) : []);
}

export async function retrieveLikedNeighbors({ profile, client }) {
	const settled = await Promise.allSettled(positiveTracks(profile).map((track) =>
		search(client, `${asArray(track.artists)[0] ?? ''} ${track.title}`, 12)
	));
	return settled.flatMap((item) => item.status === 'fulfilled' ? item.value : []);
}

export async function retrieveCurrentSimilar({ context, client }) {
	const current = context?.currentTrack;
	if (!current) return [];
	const id = current.raw?.id;
	if (id && typeof client?.getJson === 'function') {
		const result = await client.getJson(`${client.apiBase}/simi/song?id=${encodeURIComponent(id)}`);
		const songs = songsFrom(result);
		if (songs.length > 0) return songs;
	}
	return search(client, `${current.artists?.[0] ?? ''} ${current.title ?? ''}`, 20);
}

export async function retrieveArtists({ profile, client }) {
	const settled = await Promise.allSettled(positiveArtists(profile).map((artist) => search(client, artist.name, 15)));
	return settled.flatMap((item) => item.status === 'fulfilled' ? item.value : []);
}
...
function sceneKeyword(context) {
	...
	if (context?.timeBand === 'late-night') return '夜晚 氛围 音乐';
	return '日常 好听 音乐';
}

export async function retrieveScenePlaylists({ context, client }) {
	return playlistCandidates(client, sceneKeyword(context), 3, 20);
}
...
export async function retrieveExploration({ client }) {
	if (!client) return [];
	if (typeof client.getJson === 'function' && typeof client.playlist === 'function') {
		const data = await client.getJson(`${client.apiBase}/personalized?limit=4`);
		const playlists = asArray(data?.result).slice(0, 4);
		const settled = await Promise.allSettled(playlists.map((item) => client.playlist(item.id, 15)));
		const songs = settled.flatMap((item) => item.status === 'fulfilled' ? asArray(item.value?.tracks).slice(0, 15) : []);
		if (songs.length > 0) return songs;
	}
	return search(client, '热门 音乐', 20);
}
