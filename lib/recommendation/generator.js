...
function usesSoftQueuePenalty(track, index, currentIndex) {
...
export function createRecommendationGenerator(options = {}) {
	...
	async function verify(entry, signal) {
	...
	async function generate(input = {}) {
		...
		try {
			...
			const protectedQueue = queue.filter((track, index) => !usesSoftQueuePenalty(track, index, currentIndex));
			const radioSeenTrackKeys = typeof player?.radioStatus === 'function'
				? player.radioStatus()?.seenTrackKeys ?? []
				: [];
			...
			for (let index = 0; index < ranked.length; index += verifyConcurrency) {
				...
				const settled = await Promise.all(batch.map((entry) => verify(entry, signal)));
				if (signal?.aborted) throw signal.reason;
