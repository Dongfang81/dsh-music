...
export function createRecommendationCoordinator(options = {}) {
	...
	function makePlan(tracks, context, snapshot, current, existingQueue) {
	...
	async function recommend(input = {}) {
		...
		try {
			...
			const baseQueue = [...(player?.state?.queue ?? [])];
			const context = contextBuilder({ ...input, profile: snapshot, currentTrack: current, queue: baseQueue });
			...
			const initialPlan = makePlan(verified, context, snapshot, current, baseQueue);
			...
			if (remaining.length > 0) {
				const background = setTimeout(async () => {
					try {
						...
						const finalPlan = makePlan([...verified, ...more], context, snapshot, current, baseQueue);
