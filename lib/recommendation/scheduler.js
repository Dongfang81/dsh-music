...
export function createRecommendationScheduler(options = {}) {
	...
	function configureTimerLifetime(handle) {
		if (idleWaiters.size > 0) handle?.ref?.();
		else handle?.unref?.();
	}
	...
	function launch() {
		...
		running = Promise.resolve()
			.then(() => generate({ reasons: batch, signal: controller.signal }))
			.then(() => { lastError = null; })
			.catch((error) => {
	...
	function schedule(reason, scheduleOptions = {}) {
		if (disposed) return false;
		if (reason) reasons.add(String(reason));
		if (scheduleOptions.urgent) {
			if (timer) { clearTimer(timer); timer = null; }
			if (running) { rerunRequested = true; return true; }
			launch();
			return true;
		}
		if (running) {
	...
	function whenIdle() {
		...
		// Background scheduling stays unrefed, but an explicit waiter is a
		// request to keep this debounce alive until the scheduled work settles.
		timer?.ref?.();
	...
	function dispose() {
		if (disposed) return false;
		disposed = true;
		activeController?.abort(new Error('recommendation scheduler disposed'));
