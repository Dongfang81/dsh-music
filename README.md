...
Moony 会在后台维护一个 **60 首**的推荐池。点击「推荐」后，**每次取出 30 首**，确认第一首可播放后替换整个播放列表，并从新一批开始播放——包括手动添加的旧队列也会被替换。听到本批末尾时，自动接上下一批 30 首并移除旧批次，无需反复点击推荐；若新批次暂不可用，则保留原队列并给出提示。
...
首次安装或推荐池尚未准备好时，点击会提示仍在准备，不会把空结果写入队列；请求处理期间会暂时防止重复点击。点击推荐会收起搜索结果并展开播放列表；已打开的收藏列表仍可同时查看。推荐池只保存歌曲身份和排序信息，不持久化临时播放链接。
...
当前通过 [GitHub Releases](https://github.com/Dongfang81/dsh-music/releases) 发布，尚未发布到 npm registry；请使用上述 GitHub 安装入口。
